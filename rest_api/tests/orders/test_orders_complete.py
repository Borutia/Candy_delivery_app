...
class OrdersCompleteTestCase(TestCase):
    def setUp(self):
        ...
        self.client.post('/orders/assign', data=courier, content_type='application/json')

    def test_missing_data(self):
        """Обращение к обработчику с пустым телом, проверка статуса 400"""
        response = self.client.post('/orders/complete', content_type='application/json')
    ...
    def test_missing_field(self):
        ...
        response = self.client.post('/orders/complete', data=data, content_type='application/json')
        self.assertEqual(response.status_code, 400)

    def test_order_not_assign(self):
        """Обращение к обработчику с неназначенным или несуществующим заказом, проверка статуса 400"""
        data = {
            "courier_id": 1,
            "order_id": 500,
            "complete_time": "2121-03-17T09:53:11.649422Z"
    ...
    def test_complete_time_less(self):
        """Загрузка данных когда время выполненния заказа раньше, чем время назначения, проверка статуса 400"""
        data = {
            "courier_id": 1,
            "order_id": 5,
    ...
    def test_idempotence(self):
        ...
        first_response = self.client.post('/orders/complete', data=data, content_type='application/json')
        self.assertEqual(first_response.status_code, 200)
        second_response = self.client.post('/orders/complete', data=data, content_type='application/json')
